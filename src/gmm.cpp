...
{
	M = maxScale; // maximum scale of gmm
}
...
// Expectation step, given observation x
double * GMM::expectation(std::vector<model_params> params, double x)
{
	...
	for (int i = 0; i < (int)params.size(); i++)
	{
		...
		y[i] = params[i].weight*1.0/(params[i].sigma*sqrt(2*M_PI))*exp(-pow(x-params[i].mean,2)/(2*pow(params[i].sigma,2)));
		sum = sum + y[i];
	}
   ...
   //find uk and Nk
	...
	for (int i = 0; i < obs.rows*obs.cols; i++)
	{
		for (int k = 0; k < (int)params.size(); k++)
		{
			...
			new_params[k].sigma = new_params[k].sigma + lk[i][k]*pow(obs.at<uchar>(i)-new_params[k].mean,2);
		}
		delete lk[i];
	}
	//delete lk;
